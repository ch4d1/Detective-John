...
function setStatus(studio, type, level, percentage) {
    switch(level){
        case "CLOSED":
            ...
            break;
        case "LOW":
            $("#"+studio+"_level_"+type).removeClass("alert-info alert-warning alert-danger alert-secondary").addClass("alert-info").text("Geringe Auslastung");
            $("#"+studio+"_percentage_"+type).removeClass("bg-info bg-warning bg-danger bg-secondary").addClass("bg-info");
            break;
        case "NORMAL":
            $("#"+studio+"_level_"+type).removeClass("alert-info alert-warning alert-danger alert-secondary").addClass("alert-warning").text("Mittlere Auslastung");
    ...
    $("#"+studio+"_percentage_"+type).css("width", percentage+"%").attr("aria-valuenow", percentage).text(percentage+"%");
}
...
function dataPARSE(studio,json){
    $.each(json.items, function(i, key){
        if(key.isCurrent == true){
            ...
            let feelpercentage = Math.round(key.percentage * 1.8);
            ...
            setStatus(studio,"org", key.level, key.percentage);
            setStatus(studio,"feel", feellevel, feelpercentage);
            return false;
        }
